...
# ==========================================
# 3. SEGUIMIENTO CORTADORAS (TÁCTIL)
# ==========================================
    ...
    if "m_seg" in st.session_state:
        ...
        with st.form("f_seg"):
            ...
            gr_s = c4.text_input("Gramaje", value=act_seg.get('gramaje', ""))
            me_s = c5.text_input("Medida Rollos", value=act_seg.get('medida_rollos', ""))
            ...
            if st.form_submit_button("💾 GUARDAR LECTURA"):
                ...
                st.success(f"Lectura de {m_s} guardada.")
    ...
    if "m_sel" in st.session_state and st.session_state.m_sel in MAQUINAS[area_actual]:
        ...
        if par:
            ...
            with st.form("inicio_op"):
                ...
                if st.form_submit_button("🚀 INICIAR PRODUCCIÓN"):
                    if op and tr:
            ...
            with st.form("cierre_op"):
                st.info(f"🟢 TRABAJANDO: {act['trabajo']} (OP: {act['op']})")
                res_fields = {}
                if area_actual == "IMPRESIÓN":
                ...
                if st.form_submit_button("💾 FINALIZAR Y GUARDAR"):
                    ...
                    # TRANSFERENCIA SELECTIVA DE DATOS TÉCNICOS (SOLO SI EXISTEN EN HISTORIAL)
                    campos_historial = {
                        "IMPRESIÓN": ["tipo_papel", "ancho", "gramaje", "medida_trabajo"],
                        "CORTE": ["tipo_papel", "ancho", "gramaje", "img_varilla", "medida_rollos"],
                        "COLECTORAS": ["tipo_papel", "medida_trabajo", "unidades_caja"],
                        "ENCUADERNACIÓN": ["formas_totales", "material", "medida"]
                    }
                    
                    for campo in campos_historial[area_actual]:
                        if campo in act:
                            final_data[campo] = act[campo]
                    
                    # INSERTAR Y BORRAR DE ACTIVOS
