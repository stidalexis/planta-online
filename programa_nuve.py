...
if menu == "🖨️ Impresión":
        ...
        with c2:
            if not actual.empty:
                with st.form("f_c_f"):
                    ...
                    md, ob = st.text_input("Motivo"), st.text_area("Observaciones")
    ...
    if maq:
        ...
        with c2:
            if not actual.empty:
                with st.form("f_co_f"):
                    st.subheader("🏁 Finalizar")
                    tc, tf = st.number_input("Cajas", 0), st.number_input("Formas", 0)
        ...
        for i, row in df_p.iterrows():
            with st.expander(f"OP {row['OP']}"):
                with st.form(f"f_e_f_{i}"):
                    ...
                    tp = st.selectbox("Presentación", ["Caja", "Paquete"])
                    pd_ = st.number_input("Desp.", 0.0)
    ...
    if perm or st.session_state.rol == "admin":
        with st.form("f_seg"):
            ...
            with c2:
                op, tr = st.text_input("OP"), st.text_input("Trabajo")
                tp = st.selectbox("Papel", MARCAS_PAPEL)
            with c3:
                nc, mr = st.number_input("Cajas", 0), st.number_input("Metros", 0.0)
                uc = st.number_input("Unid x Caja", 0)
            ob = st.text_area("Observaciones")
            if st.form_submit_button("GUARDAR"):
                guardar_dato(pd.DataFrame([{"Fecha":datetime.now().strftime("%Y-%m-%d"), "Hora_Registro":datetime.now().strftime("%H:%M"), "Turno":tu, "Máquina":ma, "OP":op, "Nombre_Trabajo":tr, "Tipo_Papel":tp, "Metros_Rollo":mr, "Unidades_Por_Caja":uc, "Num_Cajas":nc, "Observaciones":ob}]), "Seguimiento_Cortadoras")
                st.success("Guardado")
    else: st.error("Registro abierto de 1:00 PM - 2:00 PM y 9:00 PM - 10:00 PM")

elif menu == "📊 Historial":
    st.header("📊 Historial del Día")
    t1, t2, t3, t4, t5, t6 = st.tabs(["Impresión", "Corte", "Colectoras", "Encuadernación", "Paradas", "Seguimiento"])
    ...
    with t5: st.dataframe(st.session_state["Paradas_Emergencia"], use_container_width=True)
