...
def generar_op_rollos(row):
    """
    Genera un PDF para la Orden de Producción de Rollos en UNA SOLA HOJA,
    optimizando espacios para incluir información comercial y de producción.
    """
    ...
    pdf.cell(40, 3, "VERSIÓN: 1", 1, 1, 'C')
    pdf.set_xy(160, pdf.get_y())
    pdf.cell(40, 3, "OP ROLLOS", 1, 1, 'C')
    pdf.set_xy(160, pdf.get_y())
    pdf.cell(40, 3, "Enero 05 del 2026", 1, 1, 'C')

    # Título y No. OP
    pdf.set_font("Arial", 'B', 14)
    pdf.text(80, y_start + 12, "ORDEN DE PEDIDO")
    
    pdf.set_xy(160, y_start + 10)
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(40, 10, f"No. {row.get('op', '')}", 1, 1, 'C')

    # Fila R / RC / N y Datos de control
    y_id = y_start + 18
    pdf.set_xy(10, y_id)
    pdf.set_font("Arial", 'B', 8)
    pdf.cell(10, 5, "R", 1, 0, 'C'); pdf.cell(10, 5, "RC", 1, 0, 'C'); pdf.cell(10, 5, "N", 1, 0, 'C')
    ...
    pdf.set_xy(10, y_cli)
    pdf.set_font("Arial", 'B', 8)
    pdf.cell(20, 5, "Fecha Orden:", 0)
    pdf.cell(6, 5, "", 1); pdf.cell(6, 5, "", 1); pdf.cell(10, 5, "", 1)
    
    pdf.text(60, y_cli + 4, f"Cliente/Ref: {str(row.get('cliente', ''))[:50]}")
    
    y_ven = y_cli + 7
    pdf.text(10, y_ven + 4, f"Vendedor: {row.get('vendedor', '')}")
    pdf.text(80, y_ven + 4, "¿Stock Impreso?")
    pdf.set_xy(105, y_ven + 1)
    pdf.cell(3, 3, "", 1); pdf.text(109, y_ven + 4, "NO")
    pdf.set_xy(115, y_ven + 1)
    pdf.cell(3, 3, "", 1); pdf.text(119, y_ven + 4, "SI")
    
    pdf.set_xy(160, y_ven)
    pdf.cell(15, 5, "CANTIDAD", 0)
    pdf.cell(25, 5, str(row.get('cantidad_rollos', '')), 1, 1, 'C')
    ...
    pdf.set_xy(10, y_com)
    pdf.set_font("Arial", 'B', 9)
    pdf.cell(190, 5, "INFORMACIÓN COMERCIAL", 1, 1, 'C', True)

    pdf.set_font("Arial", 'B', 8)
    pdf.cell(120, 8, f" Material: {row.get('material', '')}", 1)
    pdf.cell(70, 8, f" Gramaje: {row.get('gramaje_rollos', '')} g", 1, 1)
    
    pdf.cell(25, 8, "Referencia", 1, 0, 'C')
    pdf.cell(82, 8, f" COMERCIAL: {row.get('ref_comercial', '')}", 1)
    pdf.cell(83, 8, " PRODUCCIÓN: ", 1, 1)

    pdf.cell(120, 8, f" Cantidad Rollos: {row.get('cantidad_rollos', '')}", 1)
    pdf.cell(70, 8, f" Core: {row.get('core', '')}", 1, 1)
    ...
    y_emp = pdf.get_y()
    pdf.cell(63, 5, "EMPAQUE EN BOLSA", 1, 0, 'C', True)
    pdf.cell(63, 5, "EMPAQUE EN CAJA", 1, 0, 'C', True)
    pdf.cell(64, 5, "TRANSPORTADORA", 1, 1, 'C', True)
    
    for _ in range(2):
        pdf.cell(63, 6, "", 1, 0); pdf.cell(63, 6, "", 1, 0); pdf.cell(64, 6, "", 1, 1)

    # --- OBSERVACIONES COMERCIALES ---
    pdf.set_font("Arial", 'B', 8)
    pdf.cell(190, 5, "OBSERVACIONES PREVIAS A PRODUCCIÓN", 1, 1, 'C', True)
    pdf.set_font("Arial", '', 7)
    pdf.multi_cell(190, 4, str(row.get('observaciones_rollos', ''))[:200], 1)

    # --- SECCION PRODUCCION (AHORA EN LA MISMA HOJA) ---
    y_prod_header = pdf.get_y() + 2
    pdf.set_fill_color(210, 210, 210)
    pdf.set_xy(10, y_prod_header)
    pdf.set_font("Arial", 'B', 9)
    pdf.cell(190, 5, "INFORMACIÓN PRODUCCIÓN", 1, 1, 'C', True)
    
    # Esquema Técnico reducido
    pdf.cell(190, 35, "", 1, 1)
    pdf.set_font("Arial", 'I', 7)
    pdf.text(15, y_prod_header + 10, "[ ESQUEMA TÉCNICO / GRÁFICO ]")

    # Tabla Estibas compacta
    pdf.set_font("Arial", 'B', 8)
    ...
    pdf.line(140, y_f, 195, y_f); pdf.text(140, y_f + 3, "FIRMA SUPERVISOR PROD.")
