...
def generar_op_rollos(row):
    ...
    exactamente el diseño del formulario físico "C&B PAPELES DE COLOMBIA".
    """
    pdf = FPDF(orientation='P', unit='mm', format='A4')
    pdf.add_page()
    pdf.set_auto_page_break(auto=False)

    # Colores y configuración de dibujo
    pdf.set_draw_color(0, 0, 0) # Bordes negros
    ...
    pdf.text(10, y_start + 5, "C&B PAPELES DE COLOMBIA S.A.S.")
    
    # Bloque Control Versión (Derecha Superior)
    pdf.set_font("Arial", 'B', 7)
    ...
    pdf.cell(50, 4, "Enero 05 del 2026", 1, 1, 'C')

    # Título Principal
    pdf.set_font("Arial", 'B', 16)
    pdf.text(75, y_start + 15, "ORDEN DE PEDIDO")

    # Fila de R / RC / N y Numero OP
    y_id = y_start + 22
    pdf.set_xy(70, y_id)
    pdf.set_font("Arial", 'B', 9)
    # Dibujar R RC N (Simulando la imagen)
    pdf.cell(12, 6, "R", 1, 0, 'C')
    pdf.cell(12, 6, "RC", 1, 0, 'C')
    pdf.cell(12, 6, "N", 1, 0, 'C')
    
    # Caja OP Grande
    pdf.set_xy(150, y_id - 2)
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(50, 12, f"No. {row.get('op', '')}", 1, 1, 'C')

    # OP Anterior y Fecha
    pdf.set_font("Arial", 'B', 8)
    pdf.text(70, y_id + 10, f"OP ANTERIOR N. {row.get('op_anterior', '______')}")
    pdf.text(70, y_id + 15, f"FECHA: {row.get('created_at', '')[:10]}")

    # --- DATOS CLIENTE Y VENDEDOR ---
    y_cli = y_id + 20
    pdf.set_xy(10, y_cli)
    pdf.set_font("Arial", 'B', 9)
    pdf.cell(25, 6, "Fecha Orden:", 0)
    pdf.cell(8, 6, "", 1); pdf.cell(8, 6, "", 1); pdf.cell(12, 6, "", 1) # D M A vacíos
    
    pdf.text(70, y_cli + 4, f"Cliente/Ref: {str(row.get('cliente', ''))[:40]}")
    
    y_ven = y_cli + 8
    pdf.text(10, y_ven + 4, f"Vendedor: {row.get('vendedor', '')}")
    pdf.text(70, y_ven + 4, "¿Tiene stock Impreso en almacén?")
    pdf.set_xy(120, y_ven + 1)
    pdf.cell(4, 4, "", 1); pdf.text(125, y_ven + 4, "NO")
    pdf.set_xy(135, y_ven + 1)
    pdf.cell(4, 4, "", 1); pdf.text(140, y_ven + 4, "SI")
    
    pdf.set_xy(150, y_ven)
    pdf.cell(15, 6, "CANTIDAD", 0)
    pdf.cell(35, 6, str(row.get('cantidad_rollos', '')), 1, 1, 'C')
    ...
    # Material y Gramaje
    y_tab = pdf.get_y()
    pdf.cell(120, 10, f" Material: {row.get('material', '')}", 1)
    pdf.cell(70, 10, f" Gramaje: {row.get('gramaje_rollos', '')} g", 1, 1)
    
    # Referencias
    pdf.cell(20, 10, "Referencia", 1, 0, 'C')
    pdf.cell(85, 10, f" COMERCIAL: {row.get('ref_comercial', '')}", 1)
    pdf.cell(85, 10, " PRODUCCIÓN: ", 1, 1)

    # Cantidad y Core
    pdf.cell(120, 10, f" Cantidad Rollos: {row.get('cantidad_rollos', '')}", 1)
    pdf.cell(70, 10, f" Core: {row.get('core', '')}", 1, 1)

    # Impresión
    pdf.cell(20, 10, "Impresión", 1, 0, 'C')
    pdf.cell(85, 10, f" FRENTE: {row.get('tintas_frente_rollos', 'N/A')}", 1)
    pdf.cell(85, 10, f" RESPALDO: {row.get('tintas_respaldo_rollos', 'N/A')}", 1, 1)
    ...
    # Filas de detalle empaque
    h_e = 7
    for _ in range(2):
        pdf.cell(63, h_e, "", 1, 0); pdf.cell(63, h_e, "", 1, 0); pdf.cell(64, h_e, "", 1, 1)

    # --- OBSERVACIONES ---
    pdf.set_font("Arial", 'B', 9)
    pdf.cell(190, 6, "OBSERVACIONES PREVIAS A PRODUCCIÓN", 1, 1, 'C', True)
    pdf.set_font("Arial", '', 8)
    pdf.multi_cell(190, 5, str(row.get('observaciones_rollos', '')) + "\n\n", 1)

    # --- FIRMAS ---
    y_f = pdf.get_y() + 10
    pdf.line(10, y_f, 60, y_f); pdf.text(10, y_f + 4, "FIRMA COORD. COMERCIAL")
    pdf.line(75, y_f, 125, y_f); pdf.text(75, y_f + 4, "FIRMA ASESOR")
    pdf.line(140, y_f, 195, y_f); pdf.text(140, y_f + 4, "FIRMA SUPERVISOR PROD.")

    # --- SEGUNDA PAGINA: PRODUCCION ---
    pdf.add_page()
    pdf.set_fill_color(200, 200, 200)
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(190, 8, "INFORMACIÓN PRODUCCIÓN", 1, 1, 'C', True)
    
    # Espacio para el gráfico técnico
    pdf.cell(190, 60, "", 1, 1)
    pdf.set_font("Arial", 'I', 7)
    pdf.text(15, 30, "[ ESQUEMA TÉCNICO DE MONTAJE ]")

    # Tabla Estibas (Replicando la cuadrícula 4x3 de tu imagen)
    pdf.set_font("Arial", 'B', 9)
    pdf.cell(190, 6, "REPORTE DE CAJAS POR ESTIBAS", 1, 1, 'C', True)
    
    w_e = 190 / 3
    pdf.set_font("Arial", '', 6)
    for i in range(4): # 4 filas
        pdf.cell(w_e, 10, f" ESTIBA {i*3+1} | Cant:____  H:____", 1, 0)
        pdf.cell(w_e, 10, f" ESTIBA {i*3+2} | Cant:____  H:____", 1, 0)
        pdf.cell(w_e, 10, f" ESTIBA {i*3+3} | Cant:____  H:____", 1, 1)
